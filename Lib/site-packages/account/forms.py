...
class SignupForm(forms.Form):
    #mobile, first_name, last_name, email, date_of_birth, password
    ...
    last_name = forms.CharField(
        label=_("Name"),
        max_length=30,
        widget=forms.TextInput(attrs={'class': 'halfInput', 'placeholder' : 'Last Name'}),
        required=False
    )
    ...
    first_name = forms.CharField(
        label=_("First Name"),
        max_length=30,
        widget=forms.TextInput(attrs={'class': 'halfInput', 'placeholder' : 'First Name'}),
        required=True,
    )
    ...
    def clean_email(self):
        ...
        if not qs.exists() or not settings.ACCOUNT_EMAIL_UNIQUE:
            return value
        raise forms.ValidationError(_("This email address was already registered."))
...
class LoginUsernameForm(LoginForm):
    ...
    username = forms.CharField(label=_("Mobile Number"), max_length=30)
    authentication_fail_message = _("The Mobile Number and/or password you specified are not correct.")
    ...
    def clean_username(self):
        if not re.match('^[89]\d{7}$',self.cleaned_data["username"]):
            raise forms.ValidationError(_("Invalide Mobile Number. (8 digits starting with 8 or 9)"))
